<template>
  <div class="page" style="background-color: #f7f7f7;padding:0 0 3rem 0;">
      ...
      <div class="orderGoods-GoodsInfo">
        <div class='middlediv' v-for='(goodsData, subIndex) in urlData.goodsInfo' :key="subIndex" @tap='orderClick'>
          ...
          <div class='goodNameInfo'>
            ...
            <div class='goodsPlans'>{{goodsData.detail_info}}</div>
          </div>
    ...
    <div class="orderPriceInfo">
      ...
      <div class="priceInfo-TotalPrice">
        <span>运费（快递）</span>
        <span> ¥ {{urlData.delivery_amt}} </span>
      </div>
    ...
    this.$http
      ...
      .then(res => {
        if (res.data.code == "E00000") {
          res.data.content.goodsInfo.map(item1 => {
          ...
          if (self.serve_amount > 0) {
            self.pay_amount =
              parseFloat(res.data.content.pay_amount) - self.serve_amount;
            self.voucher_flag = 1;
          } else {
