        ...
        <div v-for="(content, index) in urlData" :key="index" class="content-item" track-by="$numb">
          ...
          <div class='middleView' v-for='(goodsData,subIndex) in content.goodsInfo' :key="subIndex" @click='orderClick(content.order_no,content.order_type)'>
            <div class="goodInfoDiv">
              ...
              <view class='goodNameInfo'>
                ...
                <view class='goodsPlans' v-if="goodsData.detail_info">{{goodsData.detail_info}}</view>
              </view>
          ...
          <view class='footView'>
            <view class='totalPrice bor-1px-b'>
              ...
              <span class='goodsTotalPrice'>¥ {{content.real_AMT}}</span>
              <span class="goodsYunfei">
								（含运费{{content.delivery_amt}}）
							</span>
            </view>
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (res.data.code == "E00000") {
            res.data.content.orderinfo.map(item0 => {
              item0.goodsInfo.map(item1 => {
                if (item1.detail_info) {
                  let detailInfo = item1.detail_info;
                  var objc = JSON.parse(detailInfo);
                  if (objc.content.length > 0) {
                    var infoStr = "";
                    objc.content.map(item2 => {
                      infoStr = infoStr + item2.paramName;
                    });
                    console.log(infoStr);
                    item1.detail_info = infoStr;
                  }
                }
              });
            });
